    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <form id="csv-import-form" enctype="multipart/form-data">
                ${csrfField(csrfToken)}                <div class="modal-header">
        ...
        <div class="tab-content">
            ...
            <div class="tab-pane fade show active" id="tab-new" role="tabpanel" aria-labelledby="tab-new-tab">
                <form id="expense-create-form" class="row g-2 align-items-end" autocomplete="off"
                      data-default-cash-id="${defaultCash !== null ? defaultCash.id : ''}">
                    ...
                    <div class="col-12">
                        ...
                        <select name="category_id" class="form-select">
                            ...
                            ${each(categories, (c) => `                                <option value="${c.id}">${esc(c.name)}</option>
                            `)}                        </select>
                    </div>
                    <div class="col-6">
                        <label class="form-label small">Pagamento</label>
                        <select name="payment_method" class="form-select" required>
                            ${each(paymentMethods, (pm) => `                                <option value="${esc(pm)}" ${pm === 'card' ? 'selected' : ''}>
                                    ${esc(paymentLabels[pm] ?? pm)}                                </option>
                            `)}                        </select>
                    </div>
                    <div class="col-6">
                        ...
                        <select name="account_id" class="form-select">
                            ...
                            ${each(accounts, (a) => `                                <option value="${a.id}" data-type="${esc(a.type)}">${esc(a.name)}</option>
                            `)}                        </select>
                    </div>
                    <div class="col-12">
                ...
                <div class="modal-body">
                    <div class="row g-3">
                        ...
                        <div class="col-md-4">
                            <label class="form-label small">Pagamento</label>
                            <select name="payment_method" class="form-select" required>
                                ${each(paymentMethods, (pm) => `                                    <option value="${esc(pm)}">${esc(paymentLabels[pm] ?? pm)}</option>
                                `)}                            </select>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label small">Categoria</label>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <form id="attachment-upload-form" enctype="multipart/form-data" class="mb-0">
                    ${csrfField(csrfToken)}                    <input type="hidden" name="expense_id" value="">

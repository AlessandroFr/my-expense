...
${(contact === null) ? `    <div class="alert alert-warning">
        ...
        <a href="${esc('/contacts')}">Torna alla lista</a>.
    </div>
` : `<div class="row mb-3">
    <div class="col-12 d-flex justify-content-between align-items-center flex-wrap gap-2">
        <div>
            <a href="${esc('/contacts')}" class="text-decoration-none small text-muted">
...
<dialog id="reassign-modal" class="border-0 rounded-3 shadow p-0" style="max-width: 1100px; width: 95%">
    <form id="reassign-form" method="dialog" class="m-0">
        ${csrfField(csrfToken)}        <input type="hidden" name="source_contact_id" value="${contact.id}">
        <div class="modal-content border-0">
            <div class="modal-header">
                <h5 class="modal-title">
                    <i class="bi bi-arrow-left-right me-2"></i>
                    Riassegna movimenti di ${esc(contact.name)}                </h5>
...
<script>window.MX_CONTACT = { id: ${contact.id}, year: ${year} };</script>

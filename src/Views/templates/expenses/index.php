 ...
 *
 * Migrato da public/pages/expenses.php (sara' rimosso al cleanup C15).
 */
...
$defaultCash    = $this->defaultCash;
$contacts       = $this->contacts ?? [];
$today          = (string) ($this->today ?? date('Y-m-d'));
$paymentMethods = $this->paymentMethods ?? ['cash', 'card', 'transfer', 'other'];
                    ...
                    <div class="mb-2">
                        ...
                        <?php if (empty($accounts)): ?>
                            <div class="alert alert-warning small mb-0">
                                Nessun conto attivo. <a href="<?= $this->escape($base . '/accounts') ?>">Crea prima un conto</a>.
                            </div>
                        <?php else: ?>
                            <select name="account_id" class="form-select" required>
                                <option value="">— Seleziona conto —</option>
                                <?php foreach ($accounts as $a): ?>
                                    <option value="<?= (int) $a['id'] ?>"><?= $this->escape((string) $a['name']) ?> (<?= $this->escape((string) $a['type']) ?>)</option>
                                <?php endforeach; ?>
                            </select>
                        <?php endif; ?>
                    </div>
            ...
            <div class="tab-pane fade show active" id="tab-new" role="tabpanel" aria-labelledby="tab-new-tab">
                <form id="expense-create-form" class="row g-2 align-items-end" autocomplete="off"
                    ...
                    <div class="col-6">
                        ...
                        <input type="date" name="expense_date" class="form-control" required value="<?= $this->escape($today) ?>">
                    </div>
                    <div class="col-6">
                    ...
                    <div class="col-12">
                        ...
                        <select name="category_id" class="form-select">
                            ...
                            <?php foreach ($categories as $c): ?>
                                <option value="<?= (int) $c['id'] ?>"><?= $this->escape((string) $c['name']) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="col-6">
                        ...
                        <select name="payment_method" class="form-select" required>
                            <?php foreach ($paymentMethods as $pm): ?>
                                <option value="<?= $this->escape($pm) ?>" <?= $pm === 'card' ? 'selected' : '' ?>>
                                    <?= $this->escape($paymentLabels[$pm] ?? $pm) ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="col-6">
                        ...
                        <select name="account_id" class="form-select">
                            ...
                            <?php foreach ($accounts as $a): ?>
                                <option value="<?= (int) $a['id'] ?>" data-type="<?= $this->escape((string) $a['type']) ?>"><?= $this->escape((string) $a['name']) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="col-12">
                ...
                <form id="expenses-filters" class="row g-2 align-items-end">
                    ...
                    <div class="col-12">
                        ...
                        <select name="account_id" class="form-select form-select-sm">
                            ...
                            <?php foreach ($accounts as $a): ?>
                                <option value="<?= (int) $a['id'] ?>"><?= $this->escape((string) $a['name']) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="col-12">
                        ...
                        <select name="contact_id" class="form-select form-select-sm">
                            <option value="">Tutti</option>
                            <?php foreach ($contacts as $cn): ?>
                                <option value="<?= (int) $cn['id'] ?>"><?= $this->escape((string) $cn['name']) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="col-6">
                ...
                <div class="modal-body">
                    <div class="row g-3">
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="payment_method" class="form-select" required>
                                <?php foreach ($paymentMethods as $pm): ?>
                                    <option value="<?= $this->escape($pm) ?>"><?= $this->escape($paymentLabels[$pm] ?? $pm) ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="col-md-6">
...
<script type="module" src="<?= $this->asset('js/pages/expenses.js') ?>"></script>
<?php $this->endSection(); ?>

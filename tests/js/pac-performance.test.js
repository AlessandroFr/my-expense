...
test('la curva mette insieme versato e valore a ogni data utile', () => {
  ...
  assert.deepEqual(points.map((p) => p.versato), [100, 200, 200]);
  assert.deepEqual(points.map((p) => p.quote), [10, 15, 15]);
  assert.deepEqual(points.map((p) => p.valore), [100, 300, 375]);
});
...
test('prima del primo NAV il valore resta vuoto invece di essere inventato', () => {
  ...
  assert.deepEqual(points, [{ date: '2026-01-01', versato: 100, quote: 0, valore: null }]);
});

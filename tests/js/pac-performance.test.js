...
import { andamento, riepilogo, tir } from '../../server/pac-performance.js';

const versamento = (data, importo, nav) => ({
  contribution_date: data,
  amount: importo,
  nav,
  units: nav ? importo / nav : null,
});
...
test('senza un\'uscita e un\'entrata il rendimento non esiste', () => {
  assert.equal(tir([{ date: '2025-01-01', amount: -1000 }]), null);
  assert.equal(tir([{ date: '2025-01-01', amount: -1000 }, { date: '2026-01-01', amount: -500 }]), null);
  assert.equal(tir([]), null);
});

test('la curva mette insieme versato e valore a ogni data utile', () => {
  const contributi = [versamento('2026-01-01', 100, 10), versamento('2026-02-01', 100, 20)];
  const navs = [{ nav_date: '2026-03-01', nav: 25 }];

  const punti = andamento(contributi, navs);
  assert.deepEqual(punti.map((p) => p.date), ['2026-01-01', '2026-02-01', '2026-03-01']);
  // 10 quote a 10, poi +5 quote a 20 = 15 quote; a marzo valgono 25 l'una.
  assert.deepEqual(punti.map((p) => p.versato), [100, 200, 200]);
  assert.deepEqual(punti.map((p) => p.quote), [10, 15, 15]);
  assert.deepEqual(punti.map((p) => p.valore), [100, 300, 375]);
});
...
test('prima del primo NAV il valore resta vuoto invece di essere inventato', () => {
  ...
  const punti = andamento(contributi, []);
  assert.deepEqual(punti, [{ date: '2026-01-01', versato: 100, quote: 0, valore: null }]);
});

test('il riepilogo dice versato, valore, guadagno e ritmo', () => {
  const contributi = [versamento('2025-01-01', 1000, 10), versamento('2026-01-01', 1000, 10)];
  // 200 quote in tutto; a 11 valgono 2200.
  const r = riepilogo(contributi, [{ nav_date: '2026-01-01', nav: 11 }], '2026-01-01');

  assert.equal(r.versato, 2000);
  assert.equal(r.valore, 2200);
  assert.equal(r.guadagno, 200);
  assert.equal(r.guadagno_pct, 10);
  // Il 10% e' del montante; il ritmo annuo e' piu' alto perche' meta' dei soldi
  // e' entrata oggi.
  assert.ok(r.tir > 15 && r.tir < 25, `TIR fuori scala: ${r.tir}`);
});

test('senza NAV il riepilogo non inventa un valore', () => {
  const r = riepilogo([{ contribution_date: '2026-01-01', amount: 100, nav: null, units: null }], [], '2026-02-01');
  assert.equal(r.versato, 100);
  assert.equal(r.valore, null);
  assert.equal(r.guadagno, null);
  assert.equal(r.tir, null);
});

test('un piano senza versamenti non ha andamento', () => {
  const r = riepilogo([], [{ nav_date: '2026-01-01', nav: 11 }], '2026-01-01');
  assert.deepEqual(r.serie, []);
  assert.equal(r.versato, 0);
  assert.equal(r.valore, null);
});

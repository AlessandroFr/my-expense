...
import { confrontaVersioni, installerPiuRecente } from '../../electron/update.js';

test('le versioni si confrontano per numero, non per testo', () => {
  assert.ok(confrontaVersioni('1.10.0', '1.9.0') > 0);
  assert.ok(confrontaVersioni('1.2.0', '1.2.1') < 0);
  assert.equal(confrontaVersioni('1.2.0', '1.2.0'), 0);
  assert.ok(confrontaVersioni('1.2', '1.2.0') === 0);
});

test('si aggiorna solo verso una versione piu\' recente', () => {
  const nomi = ['MyExpense-Setup-1.0.0.exe', 'MyExpense-Setup-1.1.0.exe'];
  assert.equal(installerPiuRecente(nomi, '1.0.0').versione, '1.1.0');
  assert.equal(installerPiuRecente(nomi, '1.1.0'), null);
  assert.equal(installerPiuRecente(nomi, '2.0.0'), null);
});
...
test('fra piu\' installer nuovi vince il piu\' recente', () => {
  ...
  assert.equal(installerPiuRecente(nomi, '1.0.0').nome, 'MyExpense-Setup-1.10.0.exe');
});
...
test('gli altri file della cartella non contano', () => {
  ...
  assert.equal(installerPiuRecente(nomi, '1.0.0'), null);
});
